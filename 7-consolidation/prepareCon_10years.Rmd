...
df_dataYear<-read.csv(file="R:/working/preparing data/DeathData/final_mv_allyears.csv", header=TRUE, check.names = FALSE) #351587
...
geodata1<-do.call("rbind", geodata1) #10460503
...
geodata2<-geodata2 %>%
  ...
  rename(CT=CTNAME)
geodata<-geodata1 %>% rbind(geodata2) %>% rename(CT2=CT) #5235882
...
restrictset4<-restrictset2 %>%
  ...
  tally() %>%
  subset(n>2) %>%
  select(STUDYID) #326392 data files where decedent was in the same CT for at least one year / *after including only 933 CMAs, only 316845 deaths files where decedent was in the same CT for at least one year
...
####################subset studyids with less than 10 years to deal with these files

restrict3 <-restrict2 %>% filter(n<10) #133962
restrict3 <- restrict3[['STUDYID']]
restrict4<-restrict[restrict$STUDYID %in% restrict3,] #646940
...
remove<-duplicates[['STUDYID']] #8258 are duplicates

#remove from duplicate list since decisions have been made already for that dataset
restrict6<-restrict5[!(restrict5$STUDYID %in% remove),] #125705 are remaining
#double checking there are no duplicates
duplicates2 <-restrict6 %>% group_by(STUDYID) %>% tally()

restrict6$CT3<-restrict6$CT2 #create index for choosing CT
...
final<-restrict6 #116505 in final dataset #after excluding CMA outside of 933, there were 133963 that had less than 10 years of annual history, meaning around 38.1% of the dataset had less than 10 years residential information within the study area (could be outside the study area and inside BC)


###########################################
#####Working on dataset with 10 years and more of data to isolate only data for five years of each studyid to then decide on CT based on frequency

restrict3 <-restrict2 %>% filter(n>9)
restrict3 <- restrict3[['STUDYID']]
...
restrict5 <- restrict4 %>% group_by(STUDYID) %>%
  mutate(CT3=case_when(VERYEAR==max(VERYEAR) | 
                      ...
                      VERYEAR==(max(VERYEAR)-4) | 
                      VERYEAR==(max(VERYEAR)-5) |
                      VERYEAR==(max(VERYEAR)-6) |
                      VERYEAR==(max(VERYEAR)-7) |
                      VERYEAR==(max(VERYEAR)-8) |
                      VERYEAR==(max(VERYEAR)-9) 
                      ~ "1")) %>%
  filter(CT3==1)
...
final2<-restrict7 #235075 in final dataset #after removing CMA 933, 217617 are the final counts where there are at least 10 years of residential information within the study area, or 61.9% of the dataset has 10 years of residential information within the study area
...
countSame<-deaths %>% filter(same==1) #138554 of 351591 stayed the same or after restricting within CMA, 141367 stayed the same of 351587 (40.2%)

#look at deaths that are not included for investigation
deaths2<-df_dataYear[!(df_dataYear$STUDYID %in% listofFinal),] #10873 of 351587 #after CMA 933 restriction, 0
...
countUnknowns <- deaths %>% filter(CT==0 | CT==999.99) #3191 unknowns
countUnnownsYrs<- countUnknowns %>% group_by(DEATHDTCCYY) %>% tally() #most are from before 2011
##Replace unknowns with CTs of previous assignment
deaths$CT<-ifelse(deaths$CT==0 | deaths$CT==9999.99, deaths$CT.x, deaths$CT)

##alternative is to remove the unknowns - likely won't go with this option because we would have to remove the denominator (population)
deaths <- deaths %>% filter(!CT %in% c(0,9999.99)) #8014 unknowns
...
write.csv(finalMortality, file="R:/working/preparing data/DeathData/final_mv_allyears_sensitivity10.csv")
